> **Աղբյուր.** `Idram Merchant API New.pdf` — տեքստի արտահանում (pdf-parse)։ 2026-04-01։
...
EDP_AMOUNT Payment amount, which merchant requests from the customer. The amount must be
...
EDP_TRANS_ID
format - char(14) Payment transaction ID in Idram system. A unique number in Idram system.
EDP_TRANS_DATE
