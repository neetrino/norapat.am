> **Աղբյուր.** `Telcell API archive26-02-09.zip / WEB Магазинный шлюз v1.2.odt` — տեքստի արտահանում (ODT content.xml)։ 2026-04-01։
...
Всегда равно "PostInvoice"

issuer

Email магазина, от лица которого выставляется счёт

currency

Валюта счёта. Всегда равно "!!! Символ драм"

price

Сумма выставляемого счёта

product

Описание товара (кодируется в base64)

issuer_id

Идентификатор счёта в магазине (кодируется в base64)

valid_days

Число дней, в течении которых счёт действителен

ssn

SSN пользователя для сверки с данными платёжной системы. Опциональный параметр.

security_code

Хеш-подпись запроса. Формируется в виде: md5([секретный ключ магазина]+issuer+currency+price+product+issuer_id)
...
Язык локализации формы оплаты (am|en|ru)
...
Уникальный идентификатор счёта в системе Telcell
...
Уникальный идентификатор транзакции в системе Telcell
...
Время завершения операции в формате yyyy-MM-dd HH:mm:ss
...
Хеш-подпись запроса. Формируется в виде: md5([секретный ключ магазина]+invoice+issuer_id+payment_id+currency+sum+time+status)
